...
3. **Exception Handling**:
   - Use `try-catch` blocks in `Program.cs` and `VehicleHandler` to handle exceptions.
...
2. **Subclasses**:
   - `EngineFailureError`: Returns ex.:"Motorfel: Kontrollera motorstatus!"(sv)
   - `BrakeFailureError`: Returns ex.:"Bromsfel: Fordonet är osäkert att köra!"(sv)
   - `TransmissionError`: Returns ex.:"Växellådsproblem: Reparation krävs!"(sv)
...
3. **VehicleManagement**:
   ...
   - Includes:
     ...
     - `ExceptionHelper` for centralized exception handling.
...
### 
**Answers to Questions:** The answers to the questions are included as comments in the code (`Program.cs`).    
